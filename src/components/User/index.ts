 ...
 * @returns {Promise < void >}
 */
export async function findOneByEmail(req: AuthRequest, res: Response, next: NextFunction): Promise < void > {
    try {
        const {
            _id, email, picture, name,
        } = await UserService.findOneByEmail(req.user.email);
